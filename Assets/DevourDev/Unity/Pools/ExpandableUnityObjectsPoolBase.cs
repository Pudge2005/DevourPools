...
{
    public abstract class ExpandableUnityObjectsPoolBase<T> : IPool<T>, IExpandablePool<T>
       where T : UnityEngine.Object
    {
        ...
        public ExpandableUnityObjectsPoolBase()
            : this(0, DefaultAutoExpand, DefaultMaxAutoExpandCapacity)
        {
        }

        public ExpandableUnityObjectsPoolBase(int initialCapacity)
            : this(initialCapacity, false, 0)
        {
        }

        public ExpandableUnityObjectsPoolBase(int initialCapacity, bool autoExpand, int maxAutoExpandCapacity)
        {
            if (initialCapacity < 0)
            {
                throw new System.ArgumentException($"{nameof(initialCapacity)} < 0 ({initialCapacity}).");
            }
